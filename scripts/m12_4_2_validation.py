...
def main():
    ...
    # Build maps
    old_traces = {item["case_id"]: item for item in raw_outputs if item["case_id"] in failing_cases}
    old_evidence = {item["case_id"]: item for item in retrieval_evidence if item.get("case_id") in failing_cases}
    ...
    print("Starting SPL Validation across 3 blocked cases...")
    ...
    report_path = os.path.join(os.path.dirname(__file__), "..", "scratch", "m12_4_2_validation_results.json")
